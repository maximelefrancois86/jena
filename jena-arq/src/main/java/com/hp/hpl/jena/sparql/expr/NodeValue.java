...
import static com.hp.hpl.jena.sparql.expr.ValueSpaceClassification.VSPACE_CUSTOM;
...
public abstract class NodeValue extends ExprNode {
     ...
     *
     * See JENA-328. DatatypeFactory.newInstance() clashes with OSGi This is
     * clearly crazy, but DatatypeFactory is missing a very obvious method
     * newInstance(Classloader). The method that was added is very hard to use
...
//    }
    ...
    public static NodeValue makeNode(String lexicalForm, String langTag, String datatype) {
        ...
        if (langTag != null && datatype != null) // raise??
        {
     ...
     * false if known to be different values, throw ExprEvalException otherwise
     */
    public static boolean sameAs(NodeValue nv1, NodeValue nv2) {
        ...
        switch (compType) {
            ...
            case VSPACE_STRING:
                return XSDFuncOp.compareString(nv1, nv2) == Expr.CMP_EQUAL;
            case VSPACE_BOOLEAN:
                return XSDFuncOp.compareBoolean(nv1, nv2) == Expr.CMP_EQUAL;
            ...
            case VSPACE_UNKNOWN: {
                ...
                if (!SystemARQ.ValueExtensions) // No value extensions => raw rdfTermEquals
                {
                ...
                if (!node1.isLiteral() || !node2.isLiteral()) // One or other not a literal => not sameAs
                {
                    return false;
                }
                ...
                if (!node1.getLiteralLanguage().equals("")
                        || !node2.getLiteralLanguage().equals("")) // One had lang tags but weren't sameNode => not equals
                {
                    return false;
                }
    ...
    /**
     * Return true if the two Nodes are known to be different, return false if
     * the two Nodes are known to be the same, else throw ExprEvalException
     */
    ...
    /**
     * Return true if the two NodeValues are known to be different, return false
     * if the two NodeValues are known to be the same, else throw
     * ExprEvalException
     */
    ...
    /**
     * Compare by value if possible else compare by kind/type/lexical form Only
     * use when you want an ordering regardless of form of NodeValue, for
     * example in ORDER BY
     *
     * @param nv1
     * @param nv2
    ...
    /**
     * Compare by value (and only value) if possible. Supports <, <=, >, >= but
     * not = nor != (which are sameValueAs and notSameValueAs)
     *
     * @param nv1
     * @param nv2
     * @return negative, 0 , or positive for not possible, less than, equal,
     * greater than.
    ...
    public static int compare(NodeValue nv1, NodeValue nv2) {
        ...
        if (nv1 == null || nv2 == null) //raise(new ExprEvalException("Attempt to notSameValueAs on null") ;
        {
    ...
    public static ValueSpaceClassification classifyValueOp(NodeValue nv1, NodeValue nv2) {
        ...
        if (c1 == VSPACE_CUSTOM || c2 == VSPACE_CUSTOM) {
            return VSPACE_CUSTOM;
        }
    ...
    private static ValueSpaceClassification classifyValueSpace(NodeValue nv) {
        ...
        if (nv.isGYear()) {
            return VSPACE_G_YEAR;
        }
        if (nv.isGYearMonth()) {
            return VSPACE_G_YEARMONTH;
        }
        if (nv.isGMonth()) {
            return VSPACE_G_MONTH;
        }
        if (nv.isGMonthDay()) {
            return VSPACE_G_MONTHDAY;
        }
        if (nv.isGDay()) {
            return VSPACE_G_DAY;
        }
    ...
    /**
     * getNode - return the node form - may be null (use .asNode() to force to a
     * node)
     */
    public Node getNode() {
        return node;
    ...
    private static NodeValue nodeToNodeValue(Node node) {
        ...
        if (!node.isLiteral()) // Not a literal - no value to extract
        {
    ...
    private static NodeValue _setByValue(Node node) {
        ...
        try { // DatatypeFormatException - should not happen
            ...
            if (XSDstring.isValidLiteral(lit)) // String - plain or xsd:string, or derived datatype.
            {
            ...
            if (!datatype.equals(XSDdecimal)) {
                ...
                if (XSDinteger.isValidLiteral(lit)) {
                    ...
                    if (s.startsWith("+")) // BigInteger does not accept leading "+"
                    {
                        s = s.substring(1);
                    }
